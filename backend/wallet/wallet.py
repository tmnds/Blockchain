...
class Wallet:
    ...
    def sign(self, data):
        ...
        return decode_dss_signature(self.private_key.sign(
            json.dumps(data).encode('utf-8'), # TypeError: data must be bytes-like
    ...
    def calculate_balance(blockchain, address):
        ...
        The balance is found by adding the output values that belong to th address 
        ...
        for block in blockchain.chain:
            for transaction in block.data:
                if transaction['input']['address'] == address:
                    # Any time the address conducts a new transactions it resets
                    # its balance
